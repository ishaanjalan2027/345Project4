...
func (e *ClientEnd) GetEndname() string {
	return e.endname.(string)
}
...
func (e *ClientEnd) Call(svcMeth string, args interface{}, reply interface{}) bool {
	...
	select {
	case e.ch <- req: /* passes the request to Network's channel, which is handled by Network.ProcessReq */
		/* ok */
	case <-e.done:
		return false
	}
...
func MakeService(rcvr interface{}) *Service {
	...
	for m := 0; m < svc.typ.NumMethod(); m++ {
		...
		if method.PkgPath != "" || /* method name capitalized ? */
			mtype.NumIn() != 3 || /* method has 3 input arguments ? */
			mtype.In(2).Kind() != reflect.Ptr || /* second argument of method is a pointer ? */
			mtype.NumOut() != 0 { /* method doesn't return anything ? */
			/* the method is not suitable for a handler */
			// fmt.Printf("bad method: %v\n", mname)
		} else {
			// the method looks like a handler
			log.Printf("labrpc.go.MakeService: added RPC method '%v'", mname)
